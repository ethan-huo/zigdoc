# Zig Code Review Reference (0.16.x)
...
| `@Type(.{ .int = ... })` | `@Int(signedness, bits)` | Removed in 0.16 |
| `@Type(.{ .@"struct" = ... })` | `@Struct(...)` | Removed in 0.16 |
| `@intFromFloat(val)` | `@trunc(val)` | Deprecated in 0.16 |
| `@cImport(@cInclude(...))` | `b.addTranslateC(...)` | Deprecated in 0.16 |
| `std.fs.Dir` / `std.fs.File` | `std.Io.Dir` / `std.Io.File` | 0.16 migration |
| `std.Thread.Pool` | `std.Io.Group` / `io.async` | Removed in 0.16 |
| `std.Thread.Mutex` | `std.Io.Mutex` | 0.16 migration |
| Pointers in packed structs | `usize` + `@ptrFromInt` | Error in 0.16 |
...
try stdout.print("Hello\n", .{});
try stdout_writer.flush();  // Required
```
...
**Better:**
```zig
pub fn init(io: std.Io, gpa: Allocator, path: []const u8) !Self {
    errdefer |err| {
...
- [std-allocators.md](std-allocators.md) - Allocator selection & naming
- [style-guide.md](style-guide.md) - Style conventions
- [upgrade-0.15-to-0.16.md](upgrade-0.15-to-0.16.md) - Migration guide
