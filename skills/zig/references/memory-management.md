...
Comprehensive reference for Zig memory management, distilled from core team guidance, community experts, and battle-tested patterns. Targets Zig 0.16+.
...
| `std.heap.ArenaAllocator` | Short-lived bulk allocations (per-request, per-algorithm) | Fast alloc, no individual free, bulk deinit; **thread-safe and lock-free in 0.16+** |
| `std.heap.DebugAllocator` (was `GeneralPurposeAllocator`; GPA alias still compiles) | Development/testing | Detects double-free, UAF, leaks; slow |
...
| `std.heap.MemoryPoolUnmanaged(T)` / `MemoryPoolAlignedUnmanaged` / `MemoryPoolExtraUnmanaged` | Unmanaged variants (caller provides allocator per call) | More control; avoids storing allocator in pool struct |
...
| `std.testing.io` | I/O testing | Test reader/writer helpers for testing I/O code without real files |
...
`ArenaAllocator` is now **thread-safe and lock-free** by default. You no longer need `std.heap.ThreadSafe` to wrap an arena shared across threads — that wrapper has been removed from the standard library.
...
`std.posix.mlock` has moved. Use `std.process.lockMemory` instead:

```zig
// 0.15 and earlier:
try std.posix.mlock(slice.ptr, slice.len);
