...
```
> **DEPRECATED in 0.16.0** — use `@trunc` instead. `@trunc`/`@floor`/`@ceil`/`@round` now accept floats and return an integer when the return type is an integer.

Float → integer. Truncates fractional part. Return type inferred.
```zig
// 0.16.0+: prefer @trunc
const i: i32 = @trunc(3.7);  // i = 3
// legacy (deprecated)
...
**0.16.0 change**: Small integer types (e.g. `u24`) can now coerce to floats (e.g. `f32`) implicitly when all possible values of that integer type are exactly representable in the target float type — no explicit `@floatFromInt` required in those cases.
```zig
const f: f32 = @floatFromInt(42);
// 0.16.0+: u24 coerces to f32 implicitly (all 2^24 values are representable)
```
...
@log2(x)     // Log base 2
@log10(x)    // Log base 10
@floor(x)    // Round down (0.16.0+: returns int when result type is integer)
@ceil(x)     // Round up   (0.16.0+: returns int when result type is integer)
@round(x)    // Round to nearest (0.16.0+: returns int when result type is integer)
@trunc(x)    // Truncate toward zero (0.16.0+: returns int when result type is integer; preferred over @intFromFloat)
@mulAdd(T, a, b, c)  // Fused (a*b)+c
```

**0.16.0 change**: `@floor`, `@ceil`, `@round`, `@trunc` now forward result types — when the destination type is an integer the builtin converts float→int directly, replacing the deprecated `@intFromFloat`.
```zig
const i: i32 = @trunc(3.7);   // 3  (truncate toward zero)
const j: i32 = @floor(3.7);   // 3  (toward negative infinity)
const k: i32 = @ceil(3.2);    // 4  (toward positive infinity)
const m: i32 = @round(3.5);   // 4  (nearest, ties away from zero)
```
...
### @Type *(removed in 0.16.0)*
> **REMOVED in 0.16.0** — replaced by individual type-constructor builtins. Use the builtins below instead.

---

### Type Constructor Builtins *(new in 0.16.0)*

These replace `@Type(std.builtin.Type{ ... })` with explicit, self-documenting constructors.
...
```zig
const T = @Int(.unsigned, 24);  // equivalent to u24
...
@Struct(
    ...
    comptime attrs: []const std.builtin.Type.StructField,
) type
```
Construct a struct type at comptime.
...
@Union(
    ...
    comptime names: []const []const u8,
    comptime types: []const type,
    comptime attrs: []const std.builtin.Type.UnionField,
) type
```
Construct a union type at comptime.
...
```
Construct an enum type at comptime.

#### @Pointer
```zig
@Pointer(
    comptime size: std.builtin.Type.Pointer.Size,
    comptime attrs: std.builtin.Type.Pointer,
    comptime Element: type,
    comptime sentinel: ?*const anyopaque,
) type
```
Construct a pointer type at comptime.

#### @Fn
```zig
@Fn(
    comptime param_types: []const type,
    comptime param_attrs: []const std.builtin.Type.Fn.Param,
    comptime ReturnType: type,
    comptime attrs: std.builtin.Type.Fn,
) type
```
Construct a function type at comptime.

#### @Tuple
```zig
@Tuple(comptime field_types: []const type) type
```
Construct an anonymous tuple type.
```zig
const T = @Tuple(&.{ u32, []const u8, bool });
```
...
@EnumLiteral() type
```
Get the `@TypeOf(.some_literal)` type (the enum literal type).
...
@cImport(expr) type
```
> **DEPRECATED in 0.16.0** — use `addTranslateC` in `build.zig` instead. `@cImport` will be removed in a future release.

```zig
// build.zig (0.16.0+ preferred approach)
...
```
Include C header (inside `@cImport`). Deprecated alongside `@cImport`.

### @cDefine
```zig
@cDefine(comptime name: []const u8, value) void
```
Define C macro (inside `@cImport`). Deprecated alongside `@cImport`.

### @cUndef
```zig
@cUndef(comptime name: []const u8) void
```
Undefine C macro. Deprecated alongside `@cImport`.
